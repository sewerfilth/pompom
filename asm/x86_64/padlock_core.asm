;
; pompom padlock cipher — x86_64 core
;
; uint8_t pompom_mutate_x(state, rot, salt, sbox_id)
; uint8_t pompom_mutate_y(state, rot, salt, sbox_id)
;
; System V AMD64 ABI: edi=state, esi=rot, edx=salt, ecx=sbox_id
; Return in eax.
;
...
; ---- S-box data (contiguous: sbox_x[256] || sbox_y[256]) ----
...
; sbox_y immediately follows — no padding
sbox_y:
...
global DECL(pompom_mutate_x)
DECL(pompom_mutate_x):
    movzx   r8d, cl             ; save sbox_id (before cl is clobbered)
    movzx   eax, dil            ; state
    mov     cl, sil             ; rot into cl for variable rotate
    rol     al, cl              ; ROL8(state, rot)
    xor     al, dl              ; XOR salt
    movzx   eax, al             ; clean index
    shl     r8d, 8              ; sbox_id * 256
    lea     rcx, [sbox_x]       ; base address (RIP-relative)
    add     rcx, r8
    movzx   eax, byte [rcx+rax]
    ret

; uint8_t pompom_mutate_y(uint8_t state, uint8_t rot, uint8_t salt, uint8_t sbox_id)
global DECL(pompom_mutate_y)
DECL(pompom_mutate_y):
    movzx   r8d, cl
    movzx   eax, dil
    mov     cl, sil
    ror     al, cl              ; ROR8(state, rot)
    xor     al, dl
    movzx   eax, al
    shl     r8d, 8
    lea     rcx, [sbox_x]
