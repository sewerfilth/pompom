...
DECL(pompom_mutate_x):
    movzx   eax, byte [esp+4]      ; state
    mov     cl, byte [esp+8]       ; rot
    rol     al, cl                 ; ROL8(state, rot)
    xor     al, byte [esp+12]     ; XOR salt
    movzx   eax, al                ; clean index
    movzx   edx, byte [esp+16]    ; sbox_id
    shl     edx, 8                 ; * 256
    add     edx, eax
    movzx   eax, byte [sbox_x+edx]
    ret
